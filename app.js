...
const plans = [
  { id: "starter", tier: "Starter", name: "Starter Trading", minAmount: 5, amount: "5 AUSD et plus", daily: "0,342%", duration: "365 jours", cycle: "Objectif cycle: 125%", dividend: "0,35% / trimestre apres cycle", note: "Accessible a tous les nouveaux utilisateurs souhaitant decouvrir les solutions d'investissement de la plateforme." },
...
function renderDashboard(user) {
  ...
  const bonusTotalUsdt = document.querySelector("[data-bonus-total-usdt]");
  const bonusUsdt = document.querySelector("[data-bonus-usdt]");
  const bonusAusd = document.querySelector("[data-bonus-ausd]");
  ...
  const commissionSummaryCard = document.querySelector("[data-commission-summary-card]");
  const commissionTotalUsdt = document.querySelector("[data-commission-total-usdt]");
  const commissionUsdt = document.querySelector("[data-commission-usdt]");
  const commissionAusd = document.querySelector("[data-commission-ausd]");
  const commissionAusdUsdt = document.querySelector("[data-commission-ausd-usdt]");
  const commissionGrsc = document.querySelector("[data-commission-grsc]");
  const commissionGrscUsdt = document.querySelector("[data-commission-grsc-usdt]");
  ...
  const bonusTotals = user.bonusTotals || {};
  const bonusTotalUsdtValue = Number(user.bonusTotalUsdt ?? 0);
  const commissionTotals = user.commissionTotals || {};
  const commissionTotalUsdtValue = Number(user.commissionTotalUsdt ?? 0);
  ...
  if (bonusTotalUsdt) bonusTotalUsdt.textContent = formatUsdt(bonusTotalUsdtValue);
  if (bonusUsdt) bonusUsdt.textContent = formatUsdt(bonusTotals.USDT || 0);
  if (bonusAusd) bonusAusd.textContent = formatAusd(bonusTotals.AUSD || 0);
  ...
  if (commissionAusdUsdt) commissionAusdUsdt.textContent = `≈ ${formatUsdt(ausdToUsdt(commissionTotals.AUSD || 0))}`;
  if (commissionGrsc) commissionGrsc.textContent = formatGrsc(commissionTotals.GRSC || 0);
  if (commissionGrscUsdt) commissionGrscUsdt.textContent = `≈ ${formatUsdt(grscToUsdt(commissionTotals.GRSC || 0, user))}`;
